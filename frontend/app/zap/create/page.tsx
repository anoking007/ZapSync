...
function Modal({ index, onSelect, availableItems }: { index: number, onSelect: (props: null | { name: string; id: string; metadata: any; }) => void, availableItems: {id: string, name: string, image: string;}[] }) {
    ...
    // Add these logs
    useEffect(() => {
        console.log("Modal rendered. Current step:", step);
        console.log("Available Items (Triggers/Actions):", availableItems);
        console.log("Selected Action state:", selectedAction);
    }, [step, availableItems, selectedAction]);
            ...
            <div className="relative p-4 w-full max-w-2xl max-h-full">
                <div className="relative bg-white rounded-lg shadow ">
                    ...
                    <div className="p-4 md:p-5 space-y-4">
                        ...
                        {step === 1 && selectedAction?.id === "email-action" && (
                            <EmailSelector setMetadata={(metadata) => {
                                onSelect({ ...selectedAction, metadata });
                            }} />
                        )}
                        {step === 1 && selectedAction?.id === "solana-action" && (
                        ...
                        {step === 0 && (
                            <div>
                                {availableItems.map(({id, name, image}) => {
                                    return (
                                        <div
                                            key={id} // Add a key for list rendering
                                            onClick={() => {
                                                if (isTrigger) {
                                                    onSelect({ id, name, metadata: {} });
                                                } else {
                                                    // Add a log here to see what's selected
                                                    console.log("Action selected for step 1:", { id, name });
                                                    setSelectedAction({ id, name }); // Set selected action
                                                    setStep(1); // Set step to 1 directly, safer than s => s + 1
                                                }
                                            }}
...
function SolanaSelector({setMetadata}: {
    ...
    return <div>
        ...
        <Input label={"Amount"} type={"text"} placeholder="Amount" onChange={(e) => setAmount(e.target.value)}></Input> {/* ✅ Fixed here */}
        <div className="pt-4">
        <PrimaryButton onClick={() => {
            setMetadata({
                amount,
